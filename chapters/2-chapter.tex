...
Este procedimento é repetido até que o conjunto de pontos tenha somente um ponto. Neste caso, criamos um nó folha, contendo o ponto $p$. A Figura \ref{fig:4} ilustra todos os cortes sobre o conjunto de pontos inicial. Estes cortes foram utilizados na construção dos subconjuntos de pontos. % e utilizados para  e a construção da árvore $2D$ correspondente.% sentação dos pontos no plano com os eixos de corte para a construção da árvore $2D$.
...
\end{tikzpicture}
    \caption{Plano $P$ com a subdivisão dos pontos e as linhas cinza são os cortes dos nós da árvore $2D$. Cada valor nesta imagem é um nó da árvore da Figura \ref{fig:5}.}
    \label{fig:4}
\end{figure}
...
\end{tikzpicture}
    \caption{Árvore $2D$ construída. O nó$^2_3$ indica a área da Figura \ref{fig:6}}
    \label{fig:5}
\end{figure}
        ...
        \Function{ConstróiÁrvore2D}{$P$, $profundidade$}
            ...
            \Else
                \If{$profundidade$ é par}
                    ...
                    \State Divide $P$ em dois subconjuntos pela $x_{med}$ 
                    \State Seja $P_1$ o conjunto dos pontos à esquerda de $v$ incluindo o próprio $v$
                    \State Seja $P_2$ o conjunto de pontos à direita de $v$
                \Else
                    \State Faça uma $y$-ordenação sobre $P$
                    \State Divide $P$ em dois subconjuntos pela $y_{med}$
                    \State Seja $P_1$ o conjunto dos pontos abaixo de $v$ incluindo o próprio $v$
...
\begin{algorithm}[H]
    ...
    \Function{ConsultaEmÁrvore2D}{$r$, $R$}
        \If{$r$ é folha}
        \Return  $r$ se estiver dentro da $R$
        \Else
...
% de uma  Para o caso com  onde cada nó não folha é um ponto $k$-dimensional
...
Uma consulta 2-dimensional em $P$ é uma busca de quais pontos de $P$ estão entre uma janela de consulta $[x, x'] \times [y, y']$. Um ponto $p = (p_x, p_y)$ está dentro de um retângulo de consulta se $p_x \in [x, x']$ e $p_y \in [y, y']$. %\cite{cg_rt1}
...
Iniciamos a busca no nó raiz $nó^0_1$. Temos que inicialmente encontrar o $nó_{corte}$ e $nó^1_0$ está dentro de $[-6, 1]$.
...
Como na Figura \ref{fig:11}, agora estamos no nó$^0_1$, este não é folha, então prosseguimos. Consultaremos em profundidade iniciando pela esquerda. À esquerda de nó$^0_1$ é nó$^1_1$, não folha, e portanto checaremos se este ainda é menor que a consulta, ou seja, se a busca em profundidade pode prosseguir consultando os ramos à esquerda da árvore. $-6 \leq -6$, logo, faremos uma \Call{Busca1DEmAlcance}{nó$^2_2\rightarrow \tau_{assoc}, -8, 3$}. A árvore auxiliar do nó$^2_2$ é $\tau_{assoc}$ é a árvore que segue:
