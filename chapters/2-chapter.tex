...
Sejam os pontos da Figura , os pontos, segue a construção da árvore.
...
\begin{tikzpicture}[scale=0.33]
 ...
 \fill (15,3)  circle[radius=3pt] node[anchor=north east]{(15,3)};
  ...
  \draw[draw=red] (-5,3) rectangle (5,-8);
...
\begin{algorithm}[H]
    ...
    \Function{Busca2DEmAlcance}{$v_{corte}$, $R : [x,x'] \times [y,y']$}
        ...
        \Else
            ...
            \While{$v$ não é folha}
                \If{$x \leq v$}
                    \State \Call{Busca1DEmAlcance}{$v \rightarrow \tau_{associada}, y,y'$}
                    \State $v \leftarrow filho_{esq}(v)$
                \Else
